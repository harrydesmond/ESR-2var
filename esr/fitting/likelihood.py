...
class CCLikelihood(Likelihood):
    ...
    def __init__(self):
        ...
        self.x2var = np.zeros_like(self.xvar)


    def get_pred(self, zp1, a, eq_numpy, x2=None, **kwargs):
...
class PanthLikelihood(Likelihood):
    ...
    def get_pred(self, zp1, a, eq_numpy, x2=None, integrated=False):
        ...
        if integrated:
            x2_eval = x2 if x2 is not None else (self.x2var if self.x2var is not None else np.zeros_like(zp1))
            dL = eq_numpy(zp1, x2_eval, *a) - eq_numpy(np.ones_like(zp1), x2_eval, *a)
        else:
            ...
            x2_data = x2 if x2 is not None else (self.x2var if self.x2var is not None else np.zeros_like(self.data_x))
            if len(a) == 0:
                dL = 1 / np.sqrt(eq_numpy(self.data_x, x2_data))
            else:
                dL = 1 / np.sqrt(eq_numpy(self.data_x, x2_data, *a))
    ...
    def negloglike(self, a, eq_numpy, integrated=False):
        ...
        """
        mu_pred = self.get_pred(self.xvar, np.atleast_1d(a), eq_numpy, x2=self.x2var, integrated=integrated)
...
class MockLikelihood(Likelihood):
    ...
    def get_pred(self, zp1, a, eq_numpy, x2=None, **kwargs):
        ...
        Args:
            ...
            :a (list): parameters to subsitute into equation considered
            :eq_numpy (numpy function): function to use which gives H^2
            :x2: unused, for interface compatibility
        ...
        """
        if x2 is None:
            x2 = self.x2var if self.x2var is not None else np.zeros_like(zp1)
        return np.sqrt(eq_numpy(zp1, x2, *a))
    ...
    def negloglike(self, a, eq_numpy, **kwargs):
        ...
        """
        H = self.get_pred(self.xvar, np.atleast_1d(a), eq_numpy, x2=self.x2var)
        if not np.all(np.isreal(H)):
            return np.inf
        nll = np.sum(0.5 * (H - self.yvar) ** 2 * self.inv_cov)  # inv_cov diagonal, so is vector here
        if np.isnan(nll):
...
class MSE(Likelihood):
    ...
    def negloglike(self, a, eq_numpy, **kwargs):
        ...
        ypred = self.get_pred(self.xvar, np.atleast_1d(a), eq_numpy, x2=self.x2var)
...
class PoissonLikelihood(Likelihood):
    ...
    def __init__(self, data_file, run_name, data_dir=None, fn_set='core_maths'):
        ...
        if data.shape[1] == 2:
            # Standard 2-column format: (x, counts)
            self.xvar = data[:, 0]
            self.yvar = data[:, 1]
            self.veff = np.ones_like(self.xvar)
        else:
            # 4-column LF/SMF format: (L_sun, log10_phi, sigma, V_eff)
            # ESR models phi(x) where x = L / 10^9
            # Predicted counts = phi(x) * V_eff
            self.xvar = data[:, 0] * 1e-9
            self.veff = data[:, 3]
            self.yvar = 10**(data[:, 1]) * self.veff  # counts = phi * V_eff
        self.yerr = np.sqrt(np.maximum(self.yvar, 0))
        if self.x2var is None:
            self.x2var = np.zeros_like(self.xvar)
    ...
    def negloglike(self, a, eq_numpy, **kwargs):
        ...
        if (not np.all(np.isreal(ypred))) or (not np.all(ypred > 0)):
            return np.inf
        predicted = ypred * self.veff
        nll = np.sum(predicted - self.yvar * np.log(predicted))
        if np.isnan(nll):
            return np.inf
        return nll
