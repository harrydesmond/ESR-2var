...
from esr.fitting.sympy_symbols import y as _y_sym
...
def optimise_fun(fcn_i, likelihood, tmax, pmin, pmax, comp=0, try_integration=False, log_opt=False, max_param=4, Niter_params=[40,60], Nconv_params=[-5,20], test_success=False, ignore_previous_eqns=True):
    ...
    try:
        ...
        if ("a0" in fcn_i)==False:
            Nparams = 0
            eq_numpy = sympy.lambdify([x, _y_sym], eq, modules=["numpy"])
        ...
        bad_fun = True
        x2var = likelihood.x2var if likelihood.x2var is not None else np.zeros_like(xvar)
        for p in itertools.product([1, -1], repeat=nparam):
            if not (np.sum(np.isnan(eq_numpy(xvar, x2var, *p)))>0):
                bad_fun = False
                break
